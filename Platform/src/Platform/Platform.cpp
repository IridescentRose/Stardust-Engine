...
/**
 * This is for creating the default attributes needed by the PSP OS. 
**/
...
namespace Stardust::Platform {
	...
	/**
	 * This sets up the platform's Networking libraries if needed.
	 * 
	 * Returns a boolean on its successfulness.
	**/
	auto detail::initNetworks() -> bool {
	#if CURRENT_PLATFORM == PLATFORM_PSP
	...
	/**
	 * Closes the networking APIs and stops new connections from being made if possible.
	**/
	auto detail::closeNetworks() -> void {
	#if CURRENT_PLATFORM == PLATFORM_PSP
		//NET
		sceNetApctlTerm();
		sceNetResolverTerm();
	...
	/**
	 * This is a way to download a file over HTTP. This should be moved to the PSP specific section.
	 * TODO: Move this to PSP Platform
	**/
	auto detail::getHTTPFile(const char* url, const char* filepath) -> bool {
	#if CURRENT_PLATFORM == PLATFORM_PSP
	 ...
	 * Close a socket.
	**/
	auto detail::closeSockets(int fd) -> void {
	...
	**/
	auto detail::setBlocking(int fd, int blocking) -> bool {
	#if CURRENT_PLATFORM == PLATFORM_PSP || (CURRENT_PLATFORM == PLATFORM_NIX) || (CURRENT_PLATFORM == PLATFORM_VITA)
		if (blocking) {
